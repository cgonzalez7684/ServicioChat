...
namespace App
{
    public partial class frmChat : Form
    {
        ...
        static DateTime? fechaRecMensajeCliente;
        ...
        public static int CategoriaChat; //Indica la categorizacion del chat dada por el agente
        ...
        bool ChatCerradoPorAgente;
        Hashtable ListaRespuestas;

        Flash objFlash = new Flash();
        ...
        private void LimpiarControles()
        {
            ...
            TxtInactividadCliente.Text = string.Empty;
            ...
            try
            {
                ...
                using (ServicioChat.WebChatClient context = new ServicioChat.WebChatClient())
                {
                    ...
                    foreach (var item in LstMensajesChat)
                    {
                        ...
                        fechaRecMensajeCliente = DateTime.Now;
                        String encabezado = String.Format("[{0:hh}:{0:mm}:{0:ss}] /", fechaRecMensajeCliente) + " [" + item.Nombre.ToUpper() + "] : ";
                        String dato = "";
                        ...
                        objFlash.FlashWindowEx(this.MdiParent);
            ...
            {   
                using (ServicioChat.WebChatClient chatContext = new ServicioChat.WebChatClient())
                {
                    ...
                    if (Tipo == 1)
                    {
                        ...
                        TxtRecibidoCliente.Text = fechaRecMensajeCliente.ToString();
                        TxtDirIP.Text = infoCLiente.dirIp;
                        txtZona.Text = infoCLiente.zona;
                        TxtInactividadCliente.Text = String.Format("{0:hh}:{0:mm}:{0:ss}.000", DateTime.Now - infoCLiente.UltimoMensajeE);
                    ...
                    if (estado == -1 && !ChatCerradoPorAgente)
                    {
                        objFlash.FlashWindowEx(this.MdiParent);
        ...
        private void frmChat_Load(object sender, EventArgs e)
        {
            ...
            using (ServicioChat.WebChatClient chatContext = new ServicioChat.WebChatClient())
            {
                string respuesta3 = chatContext.LimpiarChats("WcfChatCoope");
            }
            ...
            CargaListaRespuestas();
            //ListaRespuestas = new Hashtable();
            //ListaRespuestas.Add(1, "Bienvenido a Coopecaja le atiende " + FrmMain.cNombreCom.ToUpper() + " en que le podemos servir?");
            //ListaRespuestas.Add(2, "Gracias por contactarnos, le agradecemos si califica la atención recibida para poder mejorar nuestro servicio");
        ...
        private void CargaListaRespuestas()
        {
            try
            {
                ListaRespuestas = new Hashtable();
                ...
                {
                    IQueryable<respuesta> Listado = from respuestas in context.respuesta 
                                                   select respuestas;
                    
                    foreach (var item in Listado)
                    {
                        
                        ToolStripMenuItem objTrip = new ToolStripMenuItem();
                        
                        objTrip.Text = item.cTitulo;
                        ListaRespuestas.Add(item.cTitulo, item.cRespuesta);                        
                        objTrip.Click += Evento_Click;
                        popMensajes.Items.Add(objTrip);
                        ToolStripSeparator sp = new ToolStripSeparator();
                        popMensajes.Items.Add(sp);                                               
        ...
        private void ConcluirChat(int tipo)
        {
            try
            {
                ...
                using (ServicioChat.WebChatClient context = new ServicioChat.WebChatClient())
                {
                    ...
                    //Esto indica que el chat lo esta cerrando el agente y no el cliente
                    //if ((idChat > 0) && (context.ObtenerEstadoChat(idChat, "WcfChatCoope") == 1))
                    //{
                    //    //frmCerrarChat objCerrarChat = new frmCerrarChat(this);
                    //    frmCerrarChat objCerrarChat = new frmCerrarChat();
                    //    objCerrarChat.StartPosition = FormStartPosition.CenterParent;
                    //    objCerrarChat.ShowDialog(this);
                    //    context.CerrarChat(idChat, 3, MsjUsuCierreChat, "I", "WcfChatCoope");
                    //    ChatCerradoPorAgente = true;
                    //}
                    if ((context.ObtenerEstadoChat(idChat, "WcfChatCoope") == 1))
                    {
                        //frmCerrarChat objCerrarChat = new frmCerrarChat(this);
                        frmCerrarChat objCerrarChat = new frmCerrarChat();
                        objCerrarChat.StartPosition = FormStartPosition.CenterParent;
                    ...
                    frmCategorizarChat objCategorizar = new frmCategorizarChat();
                    objCategorizar.StartPosition = FormStartPosition.CenterParent;
                    objCategorizar.ShowDialog(this);
                ...
                //        objChat.nCategoriaChat = CategoriaChat;
        ...
        private void BtnHabilitarChat_Click(object sender, EventArgs e)
        {
            ...
            inicializarVariables();
            frmPrcBuscaChat obj = new frmPrcBuscaChat(this.Location.X + 120, this.Location.Y + 100);
            obj.ShowDialog(this);
        ...
        private void TxtEscribeMensaje_MouseDown(object sender, MouseEventArgs e)
        {
           
            if (e.Button == MouseButtons.Right)
            {
                popMensajes.Show(Cursor.Position);
            }
        }
        ...
        {
            TxtEscribeMensaje.Text = string.Empty;
            TxtEscribeMensaje.Text = ListaRespuestas[1].ToString().Replace("XYZ", FrmMain.cNombreCom.ToUpper());
        ...
        private void popMensajes_Opening(object sender, CancelEventArgs e)
        {
        ...
        private void algoMasToolStripMenuItem_Click(object sender, EventArgs e)
        {
           
            
        }

        private void despedidaToolStripMenuItem1_Click(object sender, EventArgs e)
        {
        ...
        private void Evento_Click(object sender,EventArgs e){
            TxtEscribeMensaje.Text = string.Empty;
            if (sender.ToString() == "Saludo")
            {
                TxtEscribeMensaje.Text = ListaRespuestas[sender.ToString().Trim()].ToString().Replace("XYZ", FrmMain.cNombreCom.Trim());
            }
            else
            {
                TxtEscribeMensaje.Text = ListaRespuestas[sender.ToString().Trim()].ToString();
            }
        }
